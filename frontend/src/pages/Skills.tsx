                        ...
                        {selectedSkill ? (
                            ...
                            >
                                {fullLoading ? (
                                    ...
                                    <SkillDetail skill={fullSkill} onClose={() => setSelectedSkill(null)} />
                                ) : null}
                            </motion.div>
                        ) : (
...
const SkillDetail: React.FC<{ skill: Skill; onClose: () => void }> = ({ skill, onClose }) => (
    <div className="divide-y divide-white/10">
        ...
        {skill.prompt_template && (
            <div className="p-5">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <FileText className="w-3.5 h-3.5" /> Prompt Template
                </h3>
                <div className="group relative">
                    ...
                    <button
                        onClick={() => navigator.clipboard.writeText(skill.prompt_template!)}
                        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-gray-500 hover:text-violet-400 px-2 py-1 rounded bg-white/5"
                    >
                        Copy
                    </button>
                </div>
        ...
        {skill.workflow_steps && skill.workflow_steps.length > 0 && (
            <div className="p-5">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <ListOrdered className="w-3.5 h-3.5" /> Workflow
                </h3>
                <ol className="space-y-2">
                    {skill.workflow_steps.map((step, i) => (
                        <li key={i} className="flex gap-2.5 text-sm text-gray-300">
                            <span className="flex-none w-5 h-5 rounded-full bg-violet-500/20 flex items-center justify-center text-[10px] font-bold text-violet-400 mt-0.5">
                                {i + 1}
                            </span>
                            {step}
                        </li>
        ...
        {skill.example_prompts && skill.example_prompts.length > 0 && (
            <div className="p-5">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Tag className="w-3.5 h-3.5" /> Example Prompts
                </h3>
                    ...
                    {skill.example_prompts.map((p, i) => (
                        <div key={i} className="group relative bg-black/40 border border-white/5 rounded-xl p-3 hover:border-violet-500/30 transition-colors">
                            ...
                            <button
                                ...
                                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-gray-500 hover:text-violet-400 px-1.5 py-0.5 rounded bg-white/5"
                            >
                                Copy
                            </button>
                        </div>
        ...
        {/* Tools */}
        {skill.tools_tested_with && skill.tools_tested_with.length > 0 && (
            <div className="p-5">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Wrench className="w-3.5 h-3.5" /> Tested With
                </h3>
        ...
        {skill.skill_body && (
            <div className="p-5">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <FileText className="w-3.5 h-3.5" /> Full SKILL.md
                </h3>
                <pre className="text-gray-400 text-xs leading-relaxed bg-black/40 rounded-xl p-4 whitespace-pre-wrap border border-white/5 overflow-auto max-h-64">
                    {skill.skill_body}
                </pre>
            </div>
        )}
